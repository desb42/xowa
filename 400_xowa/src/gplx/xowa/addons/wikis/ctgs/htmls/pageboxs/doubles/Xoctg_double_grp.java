...
public class Xoctg_double_grp implements Bfr_arg {
	...
	public void Set_count() {
		int count = itms.Count();
		if (type_is_normal)
			lbl_ctg_text	= wiki.Msg_mgr().Val_by_key_args(Key_pagecategories, count);
		else
			lbl_hidden		= wiki.Msg_mgr().Val_by_id_args(Xol_msg_itm_.Id_ctg_tbl_hidden, count);
	}
